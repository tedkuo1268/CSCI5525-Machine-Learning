...
def svmfit(X_train, y_train, C):
    ...
    P = matrix(np.dot(y_train*X_train, (y_train*X_train).T), tc='d')
    ...
    for i in range(len(lambda_sv)):
        ...
        w = np.add(w, lambda_i*y_sv[i]*X_sv[i].reshape(feature_size, 1))

    count = 0
    for i in range(len(lambda_sv)):
        lambda_i = lambda_sv[i]
        # Find unbounded support vectors to calculate b
        if (lambda_i>0.01*lambda_max) and (lambda_i<0.999*lambda_max):    
            count += 1
            X_free_sv = X_sv[i]
            y_free_sv = y_sv[i]
            bias = y_free_sv - np.dot(w.T, X_free_sv.reshape(feature_size, 1))
            print(bias)
            break
    
    # If unbounded support vectors was not found from previous range, use the minimum support vector to calculate b
    min_lambda_sv = min(lambda_sv)
    if count==0:
        min_lambda_index = lambda_sv.index(min(lambda_sv))
...
def predict(X, model_weight, bias):
    """
    Predict the label using weight and bias
    """
    feature_size = X.shape[1]
    data_size = X.shape[0]
    y_predict_class = []
    for i in range(data_size):
        if (np.dot(model_weight.T, X[i].reshape(feature_size, 1)) + bias)>0:
            y_predict = 1
        else:
            y_predict = -1
        
        y_predict_class.append(y_predict)
    
    return y_predict_class

def accuracy(X_partitioned, y_partitioned, X_test, y_test, C):
    """
    ...
    for i in range(len(C)):
        ...
        for itr in range(10):
            ...
            model_weight, bias = svmfit(X_train, y_train, C[i])
            #print(X_valid)
            
            # Compute the train error rate in each iteration
            total_errors_train = 0
            y_train_predict = predict(X_train, model_weight, bias)
            ...
            # Compute the validation error rate in each iteration
            total_errors_valid = 0
            y_valid_predict = predict(X_valid, model_weight, bias)
            ...
            # Test performance
            total_errors_test = 0
            y_test_predict = predict(X_test, model_weight, bias)
        ...
        # Compute the average error rates of training and validation
...
if __name__ == '__main__':
    ...
    accuracy_train, accuracy_valid, accuracy_test = accuracy(X_partitioned, y_partitioned, X_test, y_test, C)
    
    # Save the output
    pd.DataFrame(accuracy_train, columns=['C', 'accuracy']).to_csv('lin_svm_train.csv', index=False, header=True)
    pd.DataFrame(accuracy_valid, columns=['C', 'accuracy']).to_csv('lin_svm_valid.csv', index=False, header=True)
    pd.DataFrame(accuracy_test, columns=['C', 'accuracy']).to_csv('lin_svm_test.csv', index=False, header=True)
    ...
    plt.savefig('linear_svm_accuracy.png')
    ...
    for data_i in data:
        if data_i[2]==1:
            class1_x.append(data_i[0])
            class1_y.append(data_i[1])
        else:
            class2_x.append(data_i[0])
            class2_y.append(data_i[1])
